...
namespace HumanResourcesAPI.Services {
    ...
    public class ShiftBuilder : IShiftBuilder {
        ...
        public async Task<BuilderResponse> GetShift_ByShiftID(byte shiftID) {
            var query = Search(_entities.Shifts, x => x.ShiftID == shiftID).Select(ProjectToModel);
            if (query.Any()) {
                return await Task.FromResult(new BuilderResponse{ Model = query.Single() }); 
            }
            else {
                return await Task.FromResult(new BuilderResponse { ValidationMessage = $"Record Not Found; Shift with shiftID = '{shiftID}' doesn't exist." }); 
            }
        }
