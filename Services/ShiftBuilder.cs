...
namespace HumanResourcesAPI.Services {
    ...
    public class ShiftBuilder : IShiftBuilder {
        ...
        public ShiftBuilder(EntitiesContext context, IMapper mapper, ILoggerManager logger) {
            _entities = context;
            _mapper = mapper;
            _logger = logger;
        }
        ...
        private Expression<Func<Shift, ShiftModel>>  ProjectToModel {
            get {
                return entity => _mapper.Map<ShiftModel>(entity);
            }
        }
        
        public IQueryable<ShiftModel> GetShifts() {
            return _entities.Shifts.Select(ProjectToModel);
        }
        ...
        public async Task<BuilderResponse> GetShift_ByShiftID(byte shiftID) {
          var query = await Search(_entities.Shifts, x => x.ShiftID == shiftID).Select(ProjectToModel)?.ToListAsync();
           ...
           return new BuilderResponse { RequestMessage = $"Record Not Found; Shift with shiftID = '{shiftID}' doesn't exist." }; 
            }
        }
        ...
        public async Task<BuilderResponse> AddShift(ShiftModel model) {
           try
           {
                 var entity = _mapper.Map<Shift>(model);
        ...
        public async Task<BuilderResponse> UpdateShift(ShiftModel model) {
          ...
          var query = Search(_entities.Shifts, x =>  x.ShiftID == model.ShiftID);
            if (!query.Any()) {
              return new BuilderResponse { RequestMessage = "Record Not Found; " + string.Format("Shift with _shiftID = '{0}' doesn't exist.",model.ShiftID)}; 
            }
           try
           {
        ...
        public async Task<BuilderResponse> DeleteShift(byte shiftID) {
          var query = Search(_entities.Shifts, x => x.ShiftID == shiftID);
            if (!query.Any()) {
              return new BuilderResponse { RequestMessage = "Record Not Found; " + string.Format("Shift with _shiftID = '{0}' doesn't exist.",shiftID)}; 
            }
