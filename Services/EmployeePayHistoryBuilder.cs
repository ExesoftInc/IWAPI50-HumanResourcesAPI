...
namespace HumanResourcesAPI.Services {
    ...
    public class EmployeePayHistoryBuilder : IEmployeePayHistoryBuilder {
        ...
        public async Task<BuilderResponse> GetEmployeePayHistory_ByBusinessEntityIDRateChangeDate(int businessEntityID, System.DateTime rateChangeDate) {
            var query = Search(_entities.EmployeePayHistories, x => x.BusinessEntityID == businessEntityID&& x.RateChangeDate == rateChangeDate).Select(ProjectToModel);
            if (query.Any()) {
                return await Task.FromResult(new BuilderResponse{ Model = query.Single() }); 
            }
            else {
                return await Task.FromResult(new BuilderResponse { ValidationMessage = $"Record Not Found; EmployeePayHistory with businessEntityID, rateChangeDate = '{businessEntityID}', '{rateChangeDate}' doesn't exist." }); 
            }
        }
