...
namespace HumanResourcesAPI.Services {
    ...
    public class DepartmentBuilder : IDepartmentBuilder {
        ...
        public DepartmentBuilder(EntitiesContext context, IMapper mapper, ILoggerManager logger) {
            _entities = context;
            _mapper = mapper;
            _logger = logger;
        }
        ...
        private Expression<Func<Department, DepartmentModel>>  ProjectToModel {
            get {
                return entity => _mapper.Map<DepartmentModel>(entity);
            }
        }
        
        public IQueryable<DepartmentModel> GetDepartments() {
            return _entities.Departments.Select(ProjectToModel);
        }
        ...
        public async Task<BuilderResponse> GetDepartment_ByDepartmentID(short departmentID) {
          var query = await Search(_entities.Departments, x => x.DepartmentID == departmentID).Select(ProjectToModel)?.ToListAsync();
           ...
           return new BuilderResponse { RequestMessage = $"Record Not Found; Department with departmentID = '{departmentID}' doesn't exist." }; 
            }
        }
        ...
        public async Task<BuilderResponse> UpdateDepartment(DepartmentModel model) {
          ...
          var query = Search(_entities.Departments, x =>  x.DepartmentID == model.DepartmentID);
            if (!query.Any()) {
              return new BuilderResponse { RequestMessage = "Record Not Found; " + string.Format("Department with _departmentID = '{0}' doesn't exist.",model.DepartmentID)}; 
            }
           try
           {
        ...
        public async Task<BuilderResponse> DeleteDepartment(short departmentID) {
          var query = Search(_entities.Departments, x => x.DepartmentID == departmentID);
            if (!query.Any()) {
              return new BuilderResponse { RequestMessage = "Record Not Found; " + string.Format("Department with _departmentID = '{0}' doesn't exist.",departmentID)}; 
            }
